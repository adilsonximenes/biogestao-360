...
def tela_admin_dashboard():
    ...
    with st.sidebar.expander("🔒 Cadastro & Manutenção", expanded=False):

        # ── Bloquear/Liberar novos cadastros ──
        cadastro_aberto = get_configuracao("cadastro_aberto", "1") == "1"
        st.markdown("#### 🆕 Novos Cadastros")
        if cadastro_aberto:
            st.success("✅ Cadastros **abertos** — usuários podem se cadastrar")
            if st.button(
                "🔒 Bloquear novos cadastros",
                use_container_width=True,
                key="btn_bloquear_cadastro",
            ):
                set_configuracao("cadastro_aberto", "0")
                st.warning("Cadastros bloqueados!")
                st.rerun()
        else:
            st.warning("🔒 Cadastros **bloqueados** — ninguém pode se cadastrar")
            if st.button(
                "🔓 Liberar cadastros",
                use_container_width=True,
                key="btn_liberar_cadastro",
            ):
                set_configuracao("cadastro_aberto", "1")
                st.success("Cadastros liberados!")
                st.rerun()

        st.markdown("---")

        # ── Modo Manutenção ──
        em_manutencao = get_configuracao("modo_manutencao", "0") == "1"
        st.markdown("#### 🔧 Modo Manutenção")
        if em_manutencao:
            st.error("🔧 App **em manutenção** — usuários veem tela de aviso")
            msg_atual = get_configuracao(
                "mensagem_manutencao",
            ...
            with col_m1:
                if st.button(
                    ...
                    set_configuracao("mensagem_manutencao", nova_msg)
                    st.success("Mensagem salva!")
            with col_m2:
                if st.button(
                    "✅ Desativar manutenção",
                    use_container_width=True,
                    key="btn_sair_manutencao",
                    type="primary",
                ):
                    set_configuracao("modo_manutencao", "0")
                    st.success("App voltou ao ar!")
                    st.rerun()
        else:
            st.success("✅ App **no ar** — funcionando normalmente")
            msg_padrao = get_configuracao(
                "mensagem_manutencao",
                "O BioGestão 360 está em manutenção programada. Voltamos em breve! 🔧",
            )
            nova_msg = st.text_area(
                ...
                value=msg_padrao,
                key="msg_manutencao_preview",
                height=80,
            )
            if st.button(
                "🔧 Ativar modo manutenção",
                use_container_width=True,
                key="btn_entrar_manutencao",
                type="secondary",
            ):
                set_configuracao("mensagem_manutencao", nova_msg)
                set_configuracao("modo_manutencao", "1")
                st.warning("Modo manutenção ativado!")
                st.rerun()
        st.caption(
            "⚠️ O modo manutenção bloqueia o acesso de todos os usuários, "
            "incluindo os logados. Somente o admin consegue ver o app normalmente."
        )
