...
def init_db():
    ...
    # Armazena flags como "cadastro_aberto" e "modo_manutencao"
    # sem precisar reiniciar o app para mudar o comportamento
    cursor.execute("""
    ...
    # Garantir valores padrão das configurações
    cursor.execute("""
        INSERT INTO configuracoes (chave, valor, descricao)
        VALUES
            ('cadastro_aberto', '1',
             'Permite novos cadastros na sidebar (1=aberto, 0=bloqueado)'),
            ('modo_manutencao', '0',
             'Coloca o app em manutenção (1=em manutenção, 0=normal)'),
            ('mensagem_manutencao',
...
def alterar_senha_usuario(usuario_id, nova_senha):
    """Altera a senha de um usuário comum (não admin)"""
    conn = get_connection()
    if not conn:
        return False
    cursor = conn.cursor()
    nova_hash = hashlib.sha256(nova_senha.encode()).hexdigest()
    cursor.execute(
        "UPDATE usuarios SET senha_hash = %s WHERE id = %s AND role != 'admin'",
...
def get_configuracao(chave, padrao="0"):
    """
    Lê uma configuração do banco.
    Retorna o valor como string, ou `padrao` se não encontrar.
    Exemplos de chaves: 'cadastro_aberto', 'modo_manutencao', 'mensagem_manutencao'
    """
    conn = get_connection()
    if not conn:
        return padrao
